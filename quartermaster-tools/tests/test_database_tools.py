...
class TestSQLiteQueryTool:
    def test_select_all(self, db_path):
        ...
        result = tool.run(database=db_path, sql="SELECT * FROM users")
    ...
    def test_max_rows_limit(self, db_path):
        tool = SQLiteQueryTool()
        result = tool.run(database=db_path, sql="SELECT * FROM users", max_rows=2)
        assert result.success is True
    ...
    def test_rejects_delete(self, db_path):
        tool = SQLiteQueryTool()
        result = tool.run(database=db_path, sql="DELETE FROM users WHERE id = 1")
        assert result.success is False

    def test_rejects_drop(self, db_path):
        tool = SQLiteQueryTool()
        result = tool.run(database=db_path, sql="DROP TABLE users")
        assert result.success is False

    def test_missing_database(self):
        tool = SQLiteQueryTool()
        result = tool.run(database="", sql="SELECT 1")
        assert result.success is False
        assert "database" in result.error.lower()

    def test_nonexistent_database_file(self):
        tool = SQLiteQueryTool()
        result = tool.run(database="/tmp/nonexistent_db_12345.db", sql="SELECT 1")
        assert result.success is False
        assert "not found" in result.error.lower()

    def test_missing_sql(self, db_path):
        tool = SQLiteQueryTool()
        result = tool.run(database=db_path, sql="")
        assert result.success is False

    def test_invalid_sql(self, db_path):
        tool = SQLiteQueryTool()
        result = tool.run(database=db_path, sql="NOT VALID SQL")
        assert result.success is False
    ...
    def test_sql_injection_prevented(self, db_path):
        ...
        result2 = tool.run(database=db_path, sql="SELECT COUNT(*) as cnt FROM users")
...
class TestSQLiteWriteTool:
    ...
    def test_missing_database(self):
        tool = SQLiteWriteTool()
        result = tool.run(database="", sql="SELECT 1", confirm=True)
        assert result.success is False

    def test_missing_sql(self, db_path):
        tool = SQLiteWriteTool()
        result = tool.run(database=db_path, sql="", confirm=True)
        assert result.success is False

    def test_invalid_sql(self, db_path):
        tool = SQLiteWriteTool()
        result = tool.run(database=db_path, sql="INVALID SQL", confirm=True)
        assert result.success is False
    ...
    def test_parameterized_insert_prevents_injection(self, db_path):
        ...
        r2 = query.run(database=db_path, sql="SELECT * FROM users WHERE name = ?", params=[malicious_name])
...
class TestSQLiteSchemaTool:
    ...
    def test_nonexistent_table(self, db_path):
        tool = SQLiteSchemaTool()
        result = tool.run(database=db_path, table="nonexistent")
        assert result.success is False
        assert "not found" in result.error.lower()

    def test_invalid_table_name(self, db_path):
        tool = SQLiteSchemaTool()
        result = tool.run(database=db_path, table="'; DROP TABLE users;--")
        assert result.success is False
        assert "invalid" in result.error.lower()

    def test_missing_database(self):
        tool = SQLiteSchemaTool()
        result = tool.run(database="")
        assert result.success is False

    def test_nonexistent_database_file(self):
        tool = SQLiteSchemaTool()
        result = tool.run(database="/tmp/no_such_file_xyz.db")
        assert result.success is False
...
class TestDatabaseToolMetadata:
    ...
    def test_write_tool_info(self):
        tool = SQLiteWriteTool()
        assert tool.name() == "sqlite_write"

    def test_schema_tool_info(self):
        tool = SQLiteSchemaTool()
        assert tool.name() == "sqlite_schema"
