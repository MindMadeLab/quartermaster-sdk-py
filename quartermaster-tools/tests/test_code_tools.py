...
class TestPythonExecutorTool:
    ...
    def setup_method(self) -> None:
        self.tool = PythonExecutorTool(timeout=10)
    ...
    def test_math_expression(self) -> None:
        ...
        result = self.tool.execute("print(2 + 3)")
        assert result.success
    ...
    def test_syntax_error(self) -> None:
        ...
        result = self.tool.execute("def incomplete(")
    ...
    def test_timeout(self) -> None:
        ...
        result = self.tool.execute("import time; time.sleep(60)", timeout=1)
        assert not result.success
    ...
    def test_code_too_long(self) -> None:
        ...
        result = self.tool.execute("x" * 200_000)
        assert not result.success
    ...
    def test_exit_code_captured(self) -> None:
        ...
        result = self.tool.execute("import sys; sys.exit(42)")
        assert not result.success
...
class TestShellExecutorTool:
    ...
    def setup_method(self) -> None:
        self.tool = ShellExecutorTool(timeout=10)
    ...
    def test_working_dir(self) -> None:
        ...
        result = self.tool.execute("pwd", working_dir="/tmp")
        assert result.success
    ...
    def test_blocked_mkfs(self) -> None:
        ...
        result = self.tool.execute("mkfs.ext4 /dev/sda1")
        assert not result.success
        assert "blocked" in result.error.lower()

    def test_blocked_dd(self) -> None:
        """dd commands are blocked."""
        result = self.tool.execute("dd if=/dev/zero of=/dev/sda")
        assert not result.success
        assert "blocked" in result.error.lower()
    ...
    def test_timeout(self) -> None:
        ...
        result = self.tool.execute("sleep 60", timeout=1)
        assert not result.success
        assert "timed out" in result.error.lower()
    ...
    def test_nonzero_exit(self) -> None:
        ...
        result = self.tool.execute("exit 1")
        assert not result.success
        assert result.data["returncode"] == 1

    def test_pipe_commands(self) -> None:
        """Pipe commands work."""
        result = self.tool.execute("echo 'abc def' | wc -w")
        assert result.success
...
class TestEvalMathTool:
    ...
    def test_addition(self) -> None:
        """Simple addition."""
        result = self.tool.evaluate("2 + 3")
        assert result.success
        assert result.data["result"] == 5

    def test_subtraction(self) -> None:
        """Subtraction."""
        result = self.tool.evaluate("10 - 4")
        assert result.success
        assert result.data["result"] == 6

    def test_multiplication(self) -> None:
        """Multiplication."""
        result = self.tool.evaluate("6 * 7")
        assert result.success
        assert result.data["result"] == 42

    def test_division(self) -> None:
        """Division returns float."""
        result = self.tool.evaluate("10 / 3")
        assert result.success
...
class TestJavaScriptExecutorTool:
    ...
    def setup_method(self) -> None:
        self.tool = JavaScriptExecutorTool(timeout=10)
    ...
    def test_hello_world(self) -> None:
        ...
        result = self.tool.execute("console.log('hello world')")
        assert result.success
        assert "hello world" in result.data["stdout"]
    ...
    def test_math(self) -> None:
        """Execute JS math."""
        result = self.tool.execute("console.log(2 + 3)")
        assert result.success
        assert "5" in result.data["stdout"]
    ...
    def test_syntax_error(self) -> None:
        ...
        result = self.tool.execute("function(")
        assert not result.success

    def test_missing_node_handling(self) -> None:
        """Clear error when node is not found."""
        with patch.object(self.tool, "prepare_command", return_value=["nonexistent_node_binary_xyz", "-e", "1"]):
            result = self.tool.run(code="console.log(1)")
        # The parent class handles FileNotFoundError
        assert not result.success
