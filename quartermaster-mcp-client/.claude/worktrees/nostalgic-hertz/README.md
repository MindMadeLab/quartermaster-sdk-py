...
[![Python Versions](https://img.shields.io/pypi/pyversions/quartermaster-mcp-client.svg)](https://pypi.org/project/quartermaster-mcp-client/)
...
The [Model Context Protocol](https://modelcontextprotocol.io/) is a standardized protocol for connecting AI models to tools, data sources, and services. `quartermaster-mcp-client` provides a clean, Pythonic API for discovering and invoking MCP-compliant servers from your Python applications.
...
```bash
pip install quartermaster-mcp-client[dev]
```
...
```python
from quartermaster_mcp_client.transports import StreamableTransport
...
cd packages/quartermaster-mcp-client
pip install -e ".[dev]"
```
...
# With coverage
pytest --cov=quartermaster_mcp_client
...
```bash
mypy src/quartermaster_mcp_client --strict
```

### Linting

```bash
ruff check src/quartermaster_mcp_client
ruff format src/quartermaster_mcp_client
```
