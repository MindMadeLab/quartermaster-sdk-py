# quartermaster-engine — Extraction TODO

The runtime execution engine for AI agent graphs. Takes a graph definition (from `quartermaster-graph`), resolves node implementations (from `quartermaster-nodes`), and orchestrates the execution: traversal, branching, merging, memory, message passing, and error handling. This is the brain that makes agent flows run.
...
- [ ] Using with quartermaster-graph (define the flow)
- [ ] Using with quartermaster-nodes (implement the behavior)
- [ ] Using with quartermaster-providers (call LLMs)
- [ ] Using with quartermaster-tools (execute tools)
...
- [x] PyPI package config: `quartermaster-engine` with hatchling
- [x] Optional extras: `quartermaster-engine[sqlite]`, `quartermaster-engine[redis]`, `quartermaster-engine[all]`
- [ ] PyPI publishing workflow
- [ ] Dependencies: `quartermaster-graph`, `quartermaster-nodes`, `quartermaster-providers`, `quartermaster-tools` (when published)
