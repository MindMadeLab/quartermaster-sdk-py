...
from qm_code_runner.schemas import PrebuildSpec
...
async def execute_code_in_container(
    ...
    volume_name = f"qm_metadata_{uuid.uuid4().hex[:12]}"
    volume = None
...
def cleanup_orphaned_containers(docker_client: docker.DockerClient) -> None:
    ...
    """
    for v in docker_client.volumes.list(filters={"name": "qm_metadata_"}):
        with contextlib.suppress(Exception):
            v.remove(force=True)
