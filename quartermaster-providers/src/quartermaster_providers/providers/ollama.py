...
class OllamaNativeProvider(_OpenAICompatOllamaProvider):
    ...
    def _httpx_kwargs(self, timeout: float = 10.0) -> dict[str, Any]:
        """Build kwargs for httpx.Client / httpx.AsyncClient with auth + headers.

        Centralises the auth wiring so every httpx call (capability
        detection, native /api/chat, streaming) goes through the same
        credentials without repeating the plumbing.
        """
        kwargs: dict[str, Any] = {"timeout": timeout}
        # _http_auth and _extra_headers are set on OllamaProvider.__init__
        # (the parent class). OllamaNativeProvider inherits them.
