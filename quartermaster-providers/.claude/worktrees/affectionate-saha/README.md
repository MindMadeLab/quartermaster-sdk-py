...
`quartermaster-providers` provides a single, consistent interface to interact with multiple Large Language Models. Instead of learning each provider's SDK, you define your request once and swap providers without code changes.

```python
from quartermaster_providers import LLMConfig, OpenAIProvider
...
```python
from quartermaster_providers import LLMConfig, AnthropicProvider
...
```python
from quartermaster_providers import ToolDefinition
...
```python
from quartermaster_providers import AbstractLLMProvider, TokenResponse, LLMConfig
...
```python
from quartermaster_providers import OpenAIProvider
