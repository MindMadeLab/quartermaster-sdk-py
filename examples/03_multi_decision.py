...
agent = (
    ...
    .on("technical")
        .instruction("Tech triage", model="claude-sonnet-4-20250514", system_instruction="Assess technical severity")
    .end()
    .on("billing")
        .instruction("Billing check", model="claude-sonnet-4-20250514", system_instruction="Look up account status")
    .end()
    .on("general")
        .instruction("General info", model="claude-sonnet-4-20250514", system_instruction="Provide general help")
    .end()
    ...
    .on("true")
        .instruction("Escalate", model="claude-sonnet-4-20250514", system_instruction="Create urgent ticket")
        .static("Alert team", text="Urgent issue!")
    .end()
    .on("false")
        .instruction("Standard response", model="claude-sonnet-4-20250514", system_instruction="Provide standard help")
    .end()
    ...
    .on("yes")
        .instruction("Thank user", model="claude-sonnet-4-20250514", system_instruction="Thank them and close")
    .end()
    .on("no")
        .instruction("Escalate to human", model="claude-sonnet-4-20250514", system_instruction="Transfer to human agent")
    .end()
    .end()
)
