...
| `internal_programs/code_runner/` | 4 code execution tools | Bridges to qm-code-runner |
...
### 2.2 Tool Discovery
- [ ] Scan Python packages for `qm_tools` entry point
...
### 3.1 Web Tools Pack (`qm-tools[web]`)
...
### 3.2 File Tools Pack (`qm-tools[files]`)
...
### 3.3 Code Tools Pack (`qm-tools[code]`)
- [ ] `RunPython` — Execute Python code (via qm-code-runner)
- [ ] `RunNode` — Execute Node.js code
- [ ] `RunShell` — Execute shell command

### 3.4 Data Tools Pack (`qm-tools[data]`)
...
- [ ] This bridges `qm-tools` with `qm-providers`
...
- [ ] Integration with qm-providers (tool calling)
- [ ] Integration with qm-mcp-client (MCP tools)
...
- `qm-providers` — Tools are formatted via `prepare_tool()` for each LLM provider
- `qm-mcp-client` — MCP tools are a type of tool discovered at runtime
- `qm-nodes` — ProgramRunner node executes tools from the registry
- `qm-code-runner` — Code execution tools delegate to code-runner
