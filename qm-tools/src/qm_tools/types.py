...
class ToolDescriptor:
    ...
    def to_openai_tools(self) -> dict[str, Any]:
        ...
        This method does **not** require ``qm-providers`` to be installed.
        """
        return {
            "type": "function",
            "function": {
    ...
    def to_tool_definition(self) -> Any:
        """Convert to ``qm_providers.ToolDefinition``.

        Requires the ``qm-providers`` package (install ``qm-tools[llm]``).

        Returns:
            A ``ToolDefinition`` TypedDict instance.

        Raises:
            ImportError: If ``qm-providers`` is not installed.
        """
        try:
            from qm_providers.types import ToolDefinition
        except ImportError:
            raise ImportError(
                "qm-providers is required for to_tool_definition(). "
                "Install it with: pip install qm-tools[llm]"
            )
