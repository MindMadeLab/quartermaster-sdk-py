...
- [ ] PyPI package: `quartermaster-graph` or `qm-graph`
- [ ] Zero external dependencies except Pydantic

## Architecture Notes

### This Is The Schema Layer, Not The Engine
- `qm-graph` defines WHAT an agent flow looks like (the blueprint)
- `qm-engine` defines HOW it executes (the runtime)
- `qm-nodes` defines WHAT each node does (the behavior)
- This separation means you can use qm-graph with a completely different engine
...
- `qm-nodes` — implements the behavior for each `NodeType`
- `qm-engine` — executes graphs defined by `qm-graph`
