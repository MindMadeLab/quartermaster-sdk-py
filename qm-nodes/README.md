...
[![PyPI version](https://img.shields.io/pypi/v/qm-nodes)](https://pypi.org/project/qm-nodes/)
...
**Dependencies**: `jinja2>=3.1`, `qm-graph` (enums), `qm-providers` (LLM config)
...
from qm_nodes.nodes import InstructionNodeV1, StartNodeV1, EndNodeV1, Decision1
...
registry = NodeRegistry()
count = registry.discover("qm_nodes.nodes")
...
### With qm-graph (graph schema)

Enums are re-exported from qm-graph for consistency:

```python
from qm_nodes.enums import AvailableNodeTypes  # Alias for qm_graph.enums.NodeType
from qm_graph.enums import NodeType             # Canonical source
```
