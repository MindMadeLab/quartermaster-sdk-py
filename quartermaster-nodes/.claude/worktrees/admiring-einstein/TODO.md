# quartermaster-nodes — Extraction TODO
...
- [ ] `Code1` — Code execution (depends on quartermaster-code-runner)
...
- `quartermaster-providers` — LLM nodes use provider abstraction for model calls
- `quartermaster-tools` — ProgramRunner uses tool abstraction
- `quartermaster-code-runner` — Code1 node uses code execution (optional)
- `quartermaster-graph` — Nodes need graph schema for traverse config
