...
[![Docker Image](https://img.shields.io/badge/Docker-Available-2496ED?logo=docker)](https://github.com/quartermaster-ai/quartermaster/pkgs/container/quartermaster-code-runner)
[![Python Versions](https://img.shields.io/pypi/pyversions/quartermaster-code-runner.svg)](https://pypi.org/project/quartermaster-code-runner/)
...
## What is quartermaster-code-runner?

`quartermaster-code-runner` is a FastAPI-based microservice that executes code in isolated Docker containers. Each execution runs in its own lightweight container with:
...
```bash
docker pull ghcr.io/quartermaster-ai/quartermaster-code-runner:latest
docker run -it \
  -v /var/run/docker.sock:/var/run/docker.sock \
  -p 8000:8000 \
  ghcr.io/quartermaster-ai/quartermaster-code-runner:latest
```
...
services:
  code-runner:
    image: ghcr.io/quartermaster-ai/quartermaster-code-runner:latest
    ports:
      - "8000:8000"
    volumes:
...
# Always run untrusted code through quartermaster-code-runner
...
```python
from quartermaster_code_runner.config import ExecutionConfig, ResourceLimits
...
```python
import asyncio
from quartermaster_code_runner import CodeRunner
...
cd packages/quartermaster-code-runner
pip install -e ".[dev]"
```
...
# With coverage
pytest --cov=quartermaster_code_runner tests/
...
```bash
ruff check src/quartermaster_code_runner
ruff format src/quartermaster_code_runner
```
