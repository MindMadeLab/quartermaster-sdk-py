...
class TestSubGraphUse:
    ...
    def test_node_counts_after_inlining(self):
        sub = GraphBuilder("Sub").start().instruction("S1").instruction("S2").end().build()
        graph = GraphBuilder("Main").start().use(sub).end().build()
...
class TestMultiDecision:
    def test_sequential_decisions(self):
        graph = (
            ...
            .on("A")
            .instruction("Handle A")
            .end()
            .on("B")
            .instruction("Handle B")
            .end()
...
class TestSwitchNode:
    ...
    def test_switch_three_branches(self):
        graph = (
            ...
            .end()
            .on("medium")
            .instruction("Medium path")
            .end()
            .on("slow")
            .instruction("Slow path")
            .end()
            .end()
            .build()
        )
...
class TestSummarizeNode:
    def test_summarize(self):
        graph = GraphBuilder("S").start().summarize("Condense", model="gpt-4o-mini").end().build()
...
class TestVarAndTextNodes:
    def test_var_node(self):
        graph = (
            GraphBuilder("Var").start().var("Set X", variable="x", expression="42").end().build()
        )
    ...
    def test_text_node(self):
        graph = (
            GraphBuilder("Txt").start().text("Template", template="Hello {{name}}").end().build()
        )
    ...
    def test_write_memory(self):
        graph = (
            ...
            .write_memory(
                "Save result",
                memory_name="output",
                variables=[{"name": "result", "expression": "done"}],
            )
            .end()
            .build()
        )
...
class TestUserDecisionNode:
    def test_user_decision(self):
        graph = (
            ...
            .on("opt1")
            .instruction("Handle opt1")
            .end()
            .on("opt2")
            .instruction("Handle opt2")
            .end()
            .end()
            .build()
        )
...
class TestComplexDecisionChain:
    def test_decision_merge_if_merge_decision(self):
        ...
        graph = (
            ...
            .on("x")
            .instruction("X handler")
            .end()
            .on("y")
            .instruction("Y handler")
            .end()
            .merge("M1")
...
class TestBranchBuilderNewNodes:
    ...
    def test_var_on_branch(self):
        graph = (
            ...
            .on("a")
            .var("Set", variable="x", expression="1")
            .end()
            .end()
            .build()
        )
    ...
    def test_read_write_memory_on_branch(self):
        graph = (
            ...
            .decision("D", options=["a"])
            .on("a")
            .read_memory("Load", memory_name="k")
            .write_memory("Save", memory_name="k", variables=[{"name": "k", "expression": "v"}])
            .end()
            .end()
            .build()
        )
    ...
    def test_user_form_on_branch(self):
        graph = (
            ...
            .decision("D", options=["a"])
            .on("a")
            .user_form("Form", parameters=[{"name": "email"}])
            .end()
            .end()
            .build()
        )
