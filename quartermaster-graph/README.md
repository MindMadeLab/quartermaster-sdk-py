...
```python
from quartermaster_graph import get_metadata_class, InstructionMetadata, NodeType
...
quartermaster-graph defines the schema; quartermaster-nodes implements behavior:

```python
# quartermaster-graph defines WHAT the graph looks like
from quartermaster_graph import GraphBuilder, NodeType

version = GraphBuilder("My Agent").start().instruction("Process").end().build()

# quartermaster-nodes defines WHAT each node does
...
quartermaster-engine consumes graph definitions and orchestrates execution:
