...
[![PyPI version](https://img.shields.io/pypi/v/qm-engine)](https://pypi.org/project/qm-engine/)
...
from qm_engine import FlowRunner, InMemoryStore
from qm_engine.nodes import SimpleNodeRegistry, NodeResult
from qm_engine.types import AgentVersion, GraphNode, GraphEdge, NodeType, TraverseOut
...
```python
from qm_engine import FlowRunner, FlowEvent, NodeStarted, NodeFinished, TokenGenerated, FlowError
...
```python
import asyncio
from qm_engine import FlowRunner, NodeStarted, TokenGenerated
...
```python
runner = FlowRunner(
    graph=agent_version,         # AgentVersion from qm-graph
...
```python
from qm_engine import ExecutionStore
...
```python
from qm_engine import FlowMemory, InMemoryStore
...
```python
from qm_engine import PersistentMemory, InMemoryPersistence
...
```python
from qm_engine.types import GraphNode, NodeType, ErrorStrategy
...
```python
from qm_engine.nodes import NodeExecutor, NodeResult
...
### With qm-graph (graph definition)
...
Bridge qm-nodes node classes to the engine's NodeExecutor protocol:

```python
from qm_nodes import NodeRegistry as QMNodeRegistry
from qm_engine.nodes import SimpleNodeRegistry, NodeExecutor, NodeResult

# Discover all qm-nodes implementations
qm_registry = QMNodeRegistry()
qm_registry.discover("qm_nodes.nodes")

# Register them with the engine
engine_registry = SimpleNodeRegistry()
# Adapt each qm-nodes class to the NodeExecutor protocol
```
